...
export default function AllDataPage() {
  ...
  const allData = [
    ...
    ...(todosData?.todoLists || []).map((item: any) => ({
      ...
      description: `${t('tasksCount', { count: item.items?.length || 0 })} | ${t('priority')}: ${translateDynamicValue('priority', item.priority)}`,
      ...
      userRole: item.assignedTo?.role ? translateDynamicValue('role', item.assignedTo.role) : (item.createdBy?.role ? translateDynamicValue('role', item.createdBy.role) : t('unknown')),
    ...
    ...(usersData?.users || []).map((item: any) => ({
      ...
      description: `${item.firstName} ${item.lastName} | ${t('role')}: ${translateDynamicValue('role', item.role)}`,
      userName: `${item.firstName} ${item.lastName}`,
      userRole: translateDynamicValue('role', item.role),
