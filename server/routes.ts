...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/users", authenticateToken, requireRole(['admin', 'manager', 'office', 'office_team']), async (req, res) => {
    try {
  ...
  // Calendar events endpoint - combines reminders and interviews for date range
  app.get("/api/calendar/events", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (!from || !to) {
        return res.status(400).json({ message: "Both 'from' and 'to' query parameters are required (YYYY-MM-DD format)" });
      }
      ...
      const reminders = await storage.getRemindersByDateRange(req.user?.id || 0, fromDate, toDate);

      // Get interviews in date range based on role
      let interviews: any[] = [];
      if (req.user!.role === 'admin') {
        const allInterviews = await storage.getInterviewRequests();
        interviews = allInterviews.filter(interview => {
          const interviewDate = new Date(interview.proposedDateTime);
        ...
        interviews = allInterviews.filter(interview => {
          const interviewDate = new Date(interview.proposedDateTime);
          const inDateRange = interviewDate >= fromDate && interviewDate <= toDate;
          const hasAccess = interview.managerId === req.user!.id || interview.managerId === null;
          return inDateRange && hasAccess;
        });
      } else if (req.user!.role === 'security') {
        const userInterviews = await storage.getInterviewRequestsByUser(req.user!.id);
        interviews = userInterviews.filter(interview => {
          const interviewDate = new Date(interview.proposedDateTime);
          return interviewDate >= fromDate && interviewDate <= toDate;
        });
      }
